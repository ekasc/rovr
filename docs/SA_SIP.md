...
The SIP check logic is implemented in `rovr sa install`, and the injection flow **was exercised end-to-end on 2026-08-24** on macOS 26.5 with exactly the relaxations above (`--without debug --without fs` + `-arm64e_preview_abi`). Injection into the live Dock, all space mutations, and cosmetics were verified (see `docs/SA.md`). The injection was re-verified on macOS 27.0 (26A428) on 2026-09-14, where the payload resolved all four Space capabilities (`0x7ff`). Build-level requirements discovered during verification: the payload AND loader must be built `-arch arm64e` to match Dock, and the loader's Mach-O caps byte must be patched from ABI v1 (`0x81`, modern clang default) to v0 (`0x80`, what Dock runs) — both handled in the crates' `build.rs`. On macOS 27 the SDK also needs a linker that understands its `arm64e.x1` slice (Xcode 27); earlier Xcode releases fail the `-arch arm64e` link. Reboot recovery remains undemonstrated.
