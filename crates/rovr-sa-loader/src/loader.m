//
// Rovr scripting-addition loader — run as ROOT; injects the Rovr payload
// dylib into Dock.app via task_for_pid + remote-thread dlopen.
//
// Usage: rovr-sa-loader <absolute-path-to-librovr_sa_payload.dylib>
//
// Requires SIP relaxations exactly as documented in docs/SA_SIP.md
// (`csrutil enable --without debug` + unrestricted filesystem; on arm64 also
// the `-arm64e_preview_abi` boot-arg). Rovr never asks for full SIP disable.
//
// :Attribution
//
// Injection technique adapted from yabai's src/osax/loader.m
// (MIT © 2019 Åsmund Vikane). The arm64e injection path is based on work by
// Jeremy Legendre (https://github.com/jslegendre).
//
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        fprintf(stderr, "usage: rovr-sa-loader <payload.dylib>\n");
        return 1;
    }
    const char *payload_path = argv[1];
    size_t path_len = strlen(payload_path);
    // Max path length is bounded by the reserved zero region in shell_code:
    // x86_64 writes the path at offset 90 into a 187-byte array, arm64 at
    // offset 168 into a 265-byte array — both leave room for exactly 96 bytes
    // of path + NUL terminator. A longer path would overflow the array.
    if (path_len == 0 || path_len > 96 || payload_path[0] != '/') {
        fprintf(stderr, "payload path must be an absolute path of at most 96 bytes\n");
        return 1;
    }
    ...
    if (task_for_pid(mach_task_self(), pid, &task) != KERN_SUCCESS) {
        fprintf(stderr, "could not retrieve task port for pid: %d (SIP debug restrictions must be relaxed, see docs/SA_SIP.md)\n", pid);
        return 1;
    }
